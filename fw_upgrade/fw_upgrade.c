			   ...
			   enum dfu_trigger_type trigger_type)
{
    ...
    if (dfu_bytes_written == 0) {
        ...
        struct dfu_status_event *dfu_event_in_progress = new_dfu_status_event();      
    ...
    if (dfu_bytes_written == file_size) {
            ...
            struct dfu_status_event *dfu_event_done = new_dfu_status_event();
            dfu_event_done->trigger_type = trigger_type;
            dfu_event_done->dfu_status = DFU_STATUS_SUCCESS_REBOOT_SCHEDULED;

            /* Submit event. */
            EVENT_SUBMIT(dfu_event_done);
            k_work_reschedule(&reboot_device_work, K_SECONDS(CONFIG_SCHEDULE_REBOOT_SECONDS));
...
static bool event_handler(const struct event_header *eh)
{
	...
	if (is_dfu_fragment_event(eh)) {
		...
		err = apply_fragment((uint8_t*)&event->fragment, event->fragment_size,
