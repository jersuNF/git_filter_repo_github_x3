...
static int parser_run(enum diagnostics_interface interface, char* cmd, uint32_t length)
{
	...
	if (cmd_len == 0) {
		...
		parser_actions.send_resp(interface, "Zephyr version is ", strlen("Zephyr version is "));
		parser_actions.send_resp(interface, KERNEL_VERSION_STRING, strlen(KERNEL_VERSION_STRING));
		parser_actions.send_resp(interface, "\r\n", strlen("\r\n"));
		return 0;
	}
	...
	if (parser_arg_is_matching("help", cmd, cmd_len)) {
		...
		parser_actions.send_resp(interface, "Dunno\r\n", strlen("Dunno\r\n"));
	}
