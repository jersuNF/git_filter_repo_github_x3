...
uint32_t time2unix(nf_time_t *time)
{
	...
	for(myWorkYear = 2016; myWorkYear < time->year; myWorkYear++)
	{
		...
		if (LeapYear(myWorkYear)) timestamp += (uint32_t)SEC_DAY; // add one day if leap year
	}

	timestamp += ((uint32_t)m_i16_mth[time->month - 1] * SEC_DAY) + ((uint32_t)(time->day - 1) * SEC_DAY)
		     + ((uint32_t)time->hour * SEC_HOUR) + ((uint32_t)time->minute * SEC_MIN) + time->second;
	if(LeapYear(time->year) && (time->month > 2)) timestamp += (uint32_t)SEC_DAY; // add one day if leap year and passed February
...
uint8_t unix2time(nf_time_t *time, uint32_t unixTimestamp)
{
	...
	for(time->year = 1970; myDaystamp<=myTmpDaystamp; time->year++)// work the years bottom up until daystamp wrap
	{
		myTmpDaystamp = myDaystamp;
		myDaystamp -= m_i16_mth[12];
		if (LeapYear(time->year)) myDaystamp--;			// remove one extra day if leap year
	}
	time->year--;

	myDaystamp = myTmpDaystamp;							//set daystamp back to this years remaining days
	for(time->month=1; myTmpDaystamp<=myDaystamp; time->month++) // work the months until timestamp wrap
	{
		myTmpDaystamp = m_i16_mth[time->month];
		if (LeapYear(time->year) && (time->month == 3)) myDaystamp--; // One extra day if past february of leap year
	}
	time->month--;
	time->day = myDaystamp - m_i16_mth[time->month - 1] + 1; //Save remaining days
	if((time->month == 3) && (time->day == 0)) {time->month--; time->day = m_i16_mth[2] - m_i16_mth[1] + 1;} //leap day
	...
	time->second = myTimestamp % SEC_MIN;				// Remaining seconds after finding minute is the resulting second

	if(time->second > SEC_MIN) return 0;
	return 1;
}
