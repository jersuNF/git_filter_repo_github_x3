 ...
 */
static bool parser_find_newline_offset(char* str, 
				       uint32_t length, 
...
static int parser_test(enum diagnostics_interface interface, char* arg)
{
	...
	if (sub_arg_len == 0) {
		const char* noargs = 
			"Specify test: lis2dw, eeprom, bme280, flash\r\n";
	...
	if (parser_arg_is_matching("lis2dw", arg, sub_arg_len)) {
		const struct device *sensor = 
			device_get_binding(
				DT_LABEL(DT_NODELABEL(movement_sensor))
				);
		...
		if (!device_is_ready(sensor)) {
			const char* readydev = 
					"LIS2DH - Device is NOT ready.\r\n";
			parser_actions.send_resp(interface, 
						readydev, strlen(readydev));
		} else {
		...
		if (eeprom_read(eeprom_dev, 0, &data, 1) != 0) {
			const char* readydev = 
				"EEPROM - Failed reading offset 0.\r\n";
			parser_actions.send_resp(interface, 
						 readydev, strlen(readydev));
		}
		char buf[10];
		...
		if (eeprom_write(eeprom_dev, 0, &data, 1) != 0) {
			const char* readydev = 
				"EEPROM - Failed writing offset 0.\r\n";
			parser_actions.send_resp(interface, 
						 readydev, strlen(readydev));
		}
		...
		const struct device *bme_dev = 
			device_get_binding(
				DT_LABEL(DT_NODELABEL(environment_sensor))
				);
 ...
 * @return 0 if ok, error code otherwise. 
 */
static int parser_run(enum diagnostics_interface interface, 
		      char* cmd, uint32_t length)
{
	char* next_arg;
	uint32_t cmd_len = parser_get_next(cmd, &next_arg);
	if (cmd_len == 0) {
		const char* info = 
			"-= Nofence collar diagnostics at your service =-\r\n"
...
uint32_t parser_handle(enum diagnostics_interface interface, 
		       uint8_t* data, 
		       uint32_t size)
{
