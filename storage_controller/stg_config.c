...
static int do_migrations()
{
	...
	if (rc == sizeof(version)) {
		...
		NCLOG_WRN(NCID,TRice( iD( 4136), "wrn: Cannot get STG_U32_MIGRATED_VERSION. version == 0 and rc: %d \n", rc));
		...
		if (rc > 0) {
			if (rc == sizeof(uint16_t)) {
				...
				if (ret != sizeof(ano_timestamp)) {
					...
					NCLOG_WRN(NCID,TRice( iD( 7931), "wrn: Failed to read STG_U32_ANO_TIMESTAMP_10002, default to version == 10002 and ret: %d \n", ret));
					version = 10002;
				}
			} else {
				version = 2004;
			...
			NCLOG_WRN(NCID,TRice( iD( 2542), "wrn: Failed to read STG_STR_MODEM_URAT_ARG_2004,default to version == 2004 and rc: %d \n", rc));
			/* Cannot read ANO/URAT, default to URAT. String will be empty */
			version = 2004;
		}
	}
	/* Do the actual NVS migration */
	NCLOG_INF(NCID,TRice( iD( 6220),"inf: Migrating NVS from %d to %d \n", version, NF_X25_VERSION_NUMBER));
	if (version == 2004) {
		...
		if (rc < 0) {
			NCLOG_ERR(NCID,TRice( iD( 1638),"err: Cannot write STG_STR_MODEM_URAT_ARG: %d \n", rc));
			return rc;
		}
	}
	/*
	...
	if (rc != sizeof(version)) {
		NCLOG_ERR(NCID,TRice( iD( 5765),"err: Cannot write STG_U32_MIGRATED_VERSION: %d \n", rc));
		return rc;
	}
...
int stg_config_init(void)
{
	...
	if (m_initialized != true) {
		...
		if (err != 0) {
			NCLOG_ERR(NCID, TRice( iD( 1108),"err: STG Config, unable to open flash area with err: %d \n", err));
			return err;
		}

		NCLOG_DBG(NCID, TRice( iD( 2870),"dbg: STG Config: AreaID%d, FaID%d, FaOff%d, FaSize%d \n", flash_area_id, (uint8_t)mp_flash_area->fa_id, (int)mp_flash_area->fa_off, (int)mp_flash_area->fa_size));

		mp_device = device_get_binding(mp_flash_area->fa_dev_name);
		if (mp_device == NULL) {
			NCLOG_ERR(NCID, TRice0( iD( 3826),"err: STG Config, unable to get device \n"));
			return -ENODEV;
		}
		...
		if (err != 0) {
			NCLOG_ERR(NCID, TRice( iD( 2868),"err: STG Config, failed to initialize NVS storage with err: %d \n", err));
			return err;
		}

		err = do_migrations();
		if (err != 0) {
			NCLOG_ERR(NCID, TRice( iD( 3271),"err: STG Config, failed to migrate with err: %d \n", err));
			return err;
		}
		m_initialized = true;
	}
...
int stg_config_u8_read(stg_config_param_id_t id, uint8_t *value)
{
	int ret;
	if (m_initialized != true) {
		NCLOG_WRN(NCID, TRice0( iD( 3543),"wrn: STG Config not initialized \n"));
		return -ENODEV;
	}

	ret = is_valid_id(id);
	if (ret != STG_U8_PARAM_TYPE) {
		NCLOG_WRN(NCID, TRice( iD( 4111),"wrn: STG u8 read, invalid id %d, access denied with ret: %d \n", (int)id, ret));
		return -ENOMSG;
	}
	...
	if (ret == -ENOENT) {
		...
		val = UINT8_MAX;
		NCLOG_WRN(NCID, TRice( iD( 6900), "wrn: STG u8 read, id-data pair does not exist at id %d \n", (int)id));
	} else if ((ret < 0) && (ret != -ENOENT)) {
		NCLOG_ERR(NCID, TRice( iD( 5382),"err: STG u8 read, failed to read storage at id %d with ret: %d \n", (int)id, ret));
		return ret;
	}
	*value = val;
	NCLOG_DBG(NCID, TRice( iD( 7360),"dbg: Read: Id=%d, Value=%d \n", (int)id, *value));
	return 0;
}
...
int stg_config_u8_write(stg_config_param_id_t id, const uint8_t value)
{
	int ret;
	if (m_initialized != true) {
		NCLOG_WRN(NCID, TRice0( iD( 5168),"wrn: STG Config not initialized \n"));
		return -ENODEV;
	}

	ret = is_valid_id(id);
	if (ret != STG_U8_PARAM_TYPE) {
		NCLOG_WRN(NCID, TRice( iD( 4546),"wrn: STG u8 write, invalid id %d, access denied with ret: %d \n", (int)id, ret));
		return -ENOMSG;
	}
	...
	if (ret < 0) {
		...
		NCLOG_ERR(NCID, TRice( iD( 5846),"err: STG u8 write, failed write to storage at id %d with ret: %d \n", (int)id, ret));
		return ret;
	}
	NCLOG_DBG(NCID, TRice( iD( 5313),"dbg: Write: Id=%d, Value=%d \n", (int)id, value));
	return 0;
}
...
int stg_config_u16_read(stg_config_param_id_t id, uint16_t *value)
{
	int ret;
	if (m_initialized != true) {
		NCLOG_WRN(NCID, TRice0( iD( 5555),"wrn: STG Config not initialized \n"));
		return -ENODEV;
	}

	ret = is_valid_id(id);
	if (ret != STG_U16_PARAM_TYPE) {
		NCLOG_WRN(NCID, TRice( iD( 3242),"wrn: STG u16 read, invalid id %d, access denied with ret: %d \n", (int)id, ret));
		return -ENOMSG;
	}
	...
	if (ret == -ENOENT) {
		...
		val = UINT16_MAX;
		NCLOG_WRN(NCID, TRice( iD( 1184), "wrn: STG u16 read, id-data pair does not exist at id %d \n", (int)id));
	} else if ((ret < 0) && (ret != -ENOENT)) {
		NCLOG_ERR(NCID, TRice( iD( 7264),"err: STG u16 read, failed to read storage at id %d with ret: %d \n", (int)id, ret));
		return ret;
	}
	*value = val;
	NCLOG_DBG(NCID, TRice( iD( 1272),"dbg: Read: Id=%d, Value=%d \n", (int)id, *value));
	return 0;
}
...
int stg_config_u16_write(stg_config_param_id_t id, const uint16_t value)
{
	int ret;
	if (m_initialized != true) {
		NCLOG_WRN(NCID, TRice0( iD( 3333),"wrn: STG Config not initialized \n"));
		return -ENODEV;
	}

	ret = is_valid_id(id);
	if (ret != STG_U16_PARAM_TYPE) {
		NCLOG_WRN(NCID, TRice( iD( 7098),"wrn: STG u16 write, invalid id %d, access denied with ret: %d \n", (int)id, ret));
		return -ENOMSG;
	}
	...
	if (ret < 0) {
		UPDATE_WRITE_ERRORS(ret);
		NCLOG_ERR(NCID, TRice( iD( 5199),"err: STG u16 write, failed write to storage at id %d with ret: %d \n", (int)id, ret));
		return ret;
	}
	NCLOG_DBG(NCID, TRice( iD( 4038),"dbg: Write: Id=%d, Value=%d \n", (int)id, value));
	return 0;
}
...
int stg_config_u32_read(stg_config_param_id_t id, uint32_t *value)
{
	int ret;
	if (m_initialized != true) {
		NCLOG_WRN(NCID, TRice0( iD( 6894),"wrn: STG Config not initialized \n"));
		return -ENODEV;
	}

	ret = is_valid_id(id);
	if (ret != STG_U32_PARAM_TYPE) {
		NCLOG_WRN(NCID, TRice( iD( 4154),"wrn: STG u32 read, invalid id %d, access denied with ret: %d \n", (int)id, ret));
		return -ENOMSG;
	}
	...
	if (ret == -ENOENT) {
		...
		val = UINT32_MAX;
		NCLOG_WRN(NCID, TRice( iD( 3970), "wrn: STG u32 read, id-data pair does not exist at id %d \n", (int)id));
	} else if ((ret < 0) && (ret != -ENOENT)) {
		NCLOG_ERR(NCID, TRice( iD( 1209),"err: STG u32 read, failed to read storage at id %d with ret: %d \n", (int)id, ret));
		return ret;
	}
	*value = val;
	NCLOG_DBG(NCID, TRice( iD( 2082),"dbg: Read: Id=%d, Value=%d \n", (int)id, *value));
	return 0;
}
...
int stg_config_u32_write(stg_config_param_id_t id, const uint32_t value)
{
	int ret;
	if (m_initialized != true) {
		NCLOG_WRN(NCID, TRice0( iD( 7183),"wrn: STG Config not initialized \n"));
		return -ENODEV;
	}

	ret = is_valid_id(id);
	if (ret != STG_U32_PARAM_TYPE) {
		NCLOG_WRN(NCID, TRice( iD( 7902),"wrn: STG u32 write, invalid id %d, access denied with ret: %d \n", (int)id, ret));
		return -ENOMSG;
	}
	...
	if (ret < 0) {
		UPDATE_WRITE_ERRORS(ret);
		NCLOG_ERR(NCID, TRice( iD( 5468),"err: STG u32 write, failed write to storage at id %d with ret: %d \n", (int)id, ret));
		return ret;
	}
	NCLOG_DBG(NCID, TRice( iD( 2275),"dbg: Write: Id=%d, Value=%d \n", (int)id, value));
	return 0;
}
...
int stg_config_str_read(stg_config_param_id_t id, char *buf, size_t bufsize, uint8_t *len)
{
	int ret;
	if (m_initialized != true) {
		NCLOG_WRN(NCID, TRice0( iD( 3827),"wrn: STG Config not initialized \n"));
		return -ENODEV;
	}

	ret = is_valid_id(id);
	if (ret != STG_STR_PARAM_TYPE) {
		NCLOG_WRN(NCID, TRice( iD( 2379), "wrn: invalid id (%d), access denied with ret: %d \n", (int)id, ret));
		return -ENOMSG;
	}
	...
	if (ret <= 0) {
		...
		NCLOG_WRN(NCID, TRice( iD( 2155), "wrn: STG string read, id-data pair does not exist at id %d with ret: %d \n", (int)id, ret));
		return ret;
	} else if (ret > bufsize) {
	...
	*len = strlen(buf);
	NCLOG_DBG(NCID, TRice( iD( 6970),"dbg: Read: Id=%d, Length=%d, Data=dynamic_string \n", (int)id, *len));
	return 0;
}
...
int stg_config_str_write(stg_config_param_id_t id, const char *str, size_t len)
{
	int ret;
	if (m_initialized != true) {
		NCLOG_WRN(NCID, TRice0( iD( 7057),"wrn: STG Config not initialized \n"));
		return -ENODEV;
	}

	ret = is_valid_id(id);
	if (ret != STG_STR_PARAM_TYPE) {
		NCLOG_WRN(NCID, TRice( iD( 7092),"wrn: STG str write, invalid id %d, access denied with ret: %d \n", (int)id, ret));
		return -ENOMSG;
	}
	...
	if (id == STG_STR_HOST_PORT) {
		...
		NCLOG_ERR(NCID, TRice( iD( 2265),"err: Invalid string key: %d \n", (int)id));
		return -EINVAL;
	}
	if (len + 1 > max_size) {
		return -EOVERFLOW;
	...
	if (ret < 0) {
		UPDATE_WRITE_ERRORS(ret);
		NCLOG_ERR(NCID, TRice( iD( 5384),"err: STG str write, failed write to storage at id %d with ret: %d \n", (int)id, ret));
		return ret;
	}
	NCLOG_DBG(NCID, TRice( iD( 7692),"dbg: Write: Id=%d, Length=%d, Data=dynamic_string \n", (int)id, len));
	return 0;
}
...
int stg_config_blob_read(stg_config_param_id_t id, uint8_t *arr, uint8_t *len)
{
	int ret;
	if (m_initialized != true) {
		NCLOG_WRN(NCID, TRice0( iD( 4341),"wrn: STG Config not initialized \n"));
		return -ENODEV;
	}

	ret = is_valid_id(id);
	if (ret != STG_BLOB_PARAM_TYPE) {
		NCLOG_WRN(NCID, TRice( iD( 6733),"wrn: STG blob read, invalid id %d, access denied with ret: %d \n", (int)id, ret));
		return -ENOMSG;
	}

	uint8_t buff[(id == STG_BLOB_BLE_KEY) ? STG_CONFIG_BLE_SEC_KEY_LEN : 0];
	if (sizeof(buff) <= 0) {
		NCLOG_WRN(NCID, TRice( iD( 6476),"wrn: STG blob read, unknown data size %d \n", sizeof(buff)));
		return -ENOMSG;
	}
	...
	if (ret == -ENOENT) {
		...
		NCLOG_WRN(NCID, TRice( iD( 3602),"wrn: STG blob read, id-data pair does not exist at id %d \n", (int)id));
		return 0;
	} else if ((ret < 0) && (ret != -ENOENT)) {
		NCLOG_ERR(NCID, TRice( iD( 3561),"err: STG blob read, failed read to storage at id %d with ret: %d \n", (int)id, ret));
		return ret;
	}
	memcpy(arr, buff, sizeof(buff));
	*len = sizeof(buff);

	NCLOG_DBG(NCID, TRice( iD( 3482),"dbg: Read: Id=%d, Length=%d, Data=0x%X%X%X%X%X%X%X%X \n", (int)id, *len, arr[0], arr[1], arr[2], arr[3], arr[4], arr[5], arr[6], arr[7]));
...
int stg_config_blob_write(stg_config_param_id_t id, const uint8_t *arr, const uint8_t len)
{
	int ret;
	if (m_initialized != true) {
		NCLOG_WRN(NCID, TRice0( iD( 5502),"wrn: STG Config not initialized \n"));
		return -ENODEV;
	}

	ret = is_valid_id(id);
	if (ret != STG_BLOB_PARAM_TYPE) {
		NCLOG_WRN(NCID, TRice( iD( 7314),"wrn: STG blob write, invalid id %d, access denied with ret: %d \n", (int)id, ret));
		return -ENOMSG;
	}
	if ((id == STG_BLOB_BLE_KEY) && (len > STG_CONFIG_BLE_SEC_KEY_LEN)) {
		NCLOG_WRN(NCID, TRice( iD( 2878),"wrn: STG blb write, incorrect size %d for id %d \n", len, (int)id));
		return -EOVERFLOW;
	}
	...
	if (ret < 0) {
		UPDATE_WRITE_ERRORS(ret);
		NCLOG_ERR(NCID, TRice( iD( 6463),"err: STG blob write, failed write to storage at id %d with ret: %d \n", (int)id, ret));
		return ret;
	}

	NCLOG_DBG(NCID, TRice( iD( 4882),"dbg: Read: Id=%d, Length=%d, Data=0x%X%X%X%X%X%X%X%X \n", (int)id, len, arr[0], arr[1], arr[2], arr[3], arr[4], arr[5], arr[6], arr[7]));
...
int stg_config_erase_all()
{
	int ret;
	if (m_initialized != true) {
		NCLOG_WRN(NCID, TRice0( iD( 4137),"wrn: STG Config not initialized \n"));
		return -ENODEV;
	}
	...
	if (ret != 0) {
		NCLOG_ERR(NCID, TRice( iD( 3971),"err: STG Config, unable to erase flash sectors, err %d \n", ret));
		return ret;
	}
	return 0;
}
...
int copy_eeprom_parameters_to_stg_flash()
{
	...
	if (ret != 0) {
		NCLOG_ERR(NCID, TRice( iD( 6578),"err: EEP(u8) Failed to read id %d, err %d \n", EEP_STG_COPY, ret));
		return ret;
	}
	...
	if (ret != 0) {
		NCLOG_ERR(NCID, TRice( iD( 2714),"err: EEP(u8) Failed to read id %d, err %d \n", EEP_UID, ret));
		return ret;
	}
	...
	if (((eep_stg_copy_val == 0) || (eep_stg_copy_val == UINT8_MAX)) && (eep_serial_no > 0) &&
		...
		for (int i = 0; i < (EEP_RESET_REASON + 1); i++) {
			...
			if (ret != 0) {
				success = -1;
				NCLOG_ERR(NCID, TRice( iD( 4204),"err: EEP(u8) Failed to read id %d, err %d \n", i, ret));
			}
			...
			if (ret != 0) {
				success = -1;
				NCLOG_ERR(NCID, TRice( iD( 5845),"err: STG(u8) Failed to read id %d, err %d \n", i, ret));
			}
			NCLOG_INF(NCID, TRice( iD( 5162),"inf: EEPvsSTG(u8): Id = %d, EEP = %d, STG = %d \n", i, eep_u8_val, stg_u8_val));
			...
			if (eep_u8_val != stg_u8_val) {
				...
				if (ret != 0) {
					success = -1;
					NCLOG_ERR(NCID, TRice( iD( 3684),"err: STG(u8) Failed to write id %d, err %d \n", i, ret));
				}
				...
				if (ret != 0) {
					success = -1;
					NCLOG_ERR(NCID, TRice( iD( 6270),"err: STG(u8) Failed to read id %d, err %d \n", i, ret));
				}
				NCLOG_INF(NCID, TRice( iD( 2422),"inf: EEPvsSTG(u8): Id = %d, EEP = %d, STG = %d \n", i, eep_u8_val, stg_u8_val));
			}
		}
		...
		for (int i = 0; i < (EEP_PRODUCT_TYPE + 1); i++) {
			...
			if (ret != 0) {
				success = -1;
				NCLOG_ERR(NCID, TRice( iD( 5451),"err: EEP(u16) Failed to read id %d, err %d \n", i, ret));
			}
			...
			if (ret != 0) {
				success = -1;
				NCLOG_ERR(NCID, TRice( iD( 7563),"err: STG(u16) Failed to read id %d(+14), err %d \n", i, ret));
			}
			NCLOG_INF(NCID, TRice( iD( 6625),"inf: EEPvsSTG(u16): Id = %d(+14), EEP = %d, STG = %d \n", i, eep_u16_val, stg_u16_val));
			...
			if (eep_u16_val != stg_u16_val) {
				...
				if (ret != 0) {
					success = -1;
					NCLOG_ERR(NCID, TRice( iD( 7967),"err: STG(u16) Failed to write id %d(+14), err %d \n", i, ret));
				}
				...
				if (ret != 0) {
					success = -1;
					NCLOG_ERR(NCID, TRice( iD( 6045),"err: STG(u16) Failed to read id %d(+14), err %d \n", i, ret));
				}
				NCLOG_INF(NCID, TRice( iD( 1155),"inf: EEPvsSTG(u16): Id = %d(+14), EEP = %d, STG = %d \n", i, eep_u16_val, stg_u16_val));
			}
		}
		...
		for (int i = 0; i < (EEP_WARN_CNT_TOT + 1); i++) {
			...
			if (ret != 0) {
				success = -1;
				NCLOG_ERR(NCID, TRice( iD( 5772),"err: EEP(u32) Failed to read id %d, err %d \n", i, ret));
			}
			...
			if (ret != 0) {
				success = -1;
				NCLOG_ERR(NCID, TRice( iD( 7989),"err: STG(u32) Failed to read id %d(+20), err %d \n", i, ret));
			}
			NCLOG_INF(NCID, TRice( iD( 7750),"inf: EEPvsSTG(u32): Id = %d(+20), EEP = %d, STG = %d \n", i, eep_u32_val, stg_u32_val));
			...
			if (eep_u32_val != stg_u32_val) {
				...
				if (ret != 0) {
					success = -1;
					NCLOG_ERR(NCID, TRice( iD( 7046),"err: STG(u32) Failed to write id %d(+20), err %d \n", i, ret));
				}
				...
				if (ret != 0) {
					success = -1;
					NCLOG_ERR(NCID, TRice( iD( 3547),"err: STG(u32) Failed to read id %d(+20), err %d \n", i, ret));
				}
				NCLOG_INF(NCID, TRice( iD( 4147),"inf: EEPvsSTG(u32): Id = %d(+20), EEP = %d, STG = %d \n", i, eep_u32_val, stg_u32_val));
			}
		}
		...
		if (ret != 0) {
			success = -1;
			NCLOG_ERR(NCID, TRice0( iD( 1156),"err: EEP Failed to read host port \n"));
		}
		NCLOG_INF(NCID, TRice0( iD( 4156),"inf: EEPvsSTG(Host port): EEP = dynamic_string \n"));

		ret = stg_config_str_write(STG_STR_HOST_PORT, port, STG_CONFIG_HOST_PORT_BUF_LEN);
		if (ret != 0) {
			success = -1;
			NCLOG_ERR(NCID, TRice0( iD( 1573),"err: STG Failed to write host port to ext flash! \n"));
		}
		...
		if (ret != 0) {
			success = -1;
			NCLOG_ERR(NCID, TRice0( iD( 6594),"err: STG Failed to read host port \n"));
		}
		NCLOG_INF(NCID, TRice( iD( 6174),"inf: EEPvsSTG(Host port): STG%d = dynamic_string \n", port_len));
		...
		if (ret != 0) {
			success = -1;
			NCLOG_ERR(NCID, TRice0( iD( 3253),"err: EEP Failed to read ble key from EEPROM \n"));
		}
		NCLOG_INF(NCID, TRice( iD( 5568),"inf: EEP(BLE key): EEP = 0x%X%X%X%X%X%X%X%X \n", ble_key[0], ble_key[1], ble_key[2], ble_key[3], ble_key[4], ble_key[5], ble_key[6], ble_key[7]));

		/* Write BLE Security key to flash */
		ret = stg_config_blob_write(STG_BLOB_BLE_KEY, ble_key, EEP_BLE_SEC_KEY_LEN);
		if (ret != 0) {
			success = -1;
			NCLOG_ERR(NCID, TRice0( iD( 2003),"err: STG Failed to write ble key to ext flash \n"));
		}
		...
		if (ret != 0) {
			success = -1;
			NCLOG_ERR(NCID, TRice0( iD( 4900),"err: STG Failed to host port \n"));
		}
		NCLOG_INF(NCID, TRice( iD( 1510),"inf: STG(BLE key): STG%d = 0x%X%X%X%X%X%X%X%X \n", key1_len, stg_ble_key[0], stg_ble_key[1], stg_ble_key[2], stg_ble_key[3], stg_ble_key[4], stg_ble_key[5], stg_ble_key[6], stg_ble_key[7]));
		...
		if (success == 0) {
			...
			if (ret != 0) {
				NCLOG_ERR(NCID, TRice( iD( 3562),"err: Failed to write to EEP_STG_COPY %i \n", ret));
			}
		}
	}
